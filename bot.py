...
DB_PATH = os.environ.get("DB_PATH", "/data/inventory.db" if os.path.isdir("/data") else "inventory.db")
...
def db_add_item(category, name, photo_id, unit):
    with _conn() as db:
        db.execute(
            "INSERT INTO items(category,name,photo_id,unit,added_at) VALUES(?,?,?,?,?)",
...
def db_new_session(user_id, stype="inventory"):
    with _conn() as db:
        c = db.execute(
            "INSERT INTO sessions(user_id,type,started_at) VALUES(?,?,?)", (user_id, stype, now())
        )
        return c.lastrowid
...
def db_writeoffs_today():
    ...
    with _conn() as db:
        return db.execute("""
            SELECT i.category, i.name, i.unit, SUM(e.quantity) as total
            FROM entries e
...
def db_add_entry(sid, item_id, qty):
    with _conn() as db:
        db.execute(
            "INSERT INTO entries(session_id,item_id,quantity,noted_at) VALUES(?,?,?,?)",
            (sid, item_id, qty, now())
        )
...
def db_latest_per_item():
    """Последний остаток по каждой позиции."""
    with _conn() as db:
        return db.execute("""
            SELECT i.category, i.name, i.unit, e.quantity,
                   s.finished_at, u.name AS user_name
            FROM entries e
            JOIN items i    ON i.id=e.item_id
            JOIN sessions s ON s.id=e.session_id
            ...
            WHERE s.finished_at IS NOT NULL
              AND e.id = (
                ...
                WHERE e2.item_id=e.item_id AND s2.finished_at IS NOT NULL
              )
            ORDER BY i.category, i.name
        """).fetchall()

def db_last_two_sessions():
    with _conn() as db:
        return db.execute("""
...
def main_kb(role):
    btns = [
        [InlineKeyboardButton("📦 Начать инвентаризацию", callback_data="inv_start")],
...
def items_kb(category, entries: dict, finish_cb="inv_finish", back_cb="back_cats", item_prefix="inv_item"):
    ...
    for it in items:
        ...
        btns.append([InlineKeyboardButton(label, callback_data=f"{item_prefix}:{it['id']}")])
...
def report_kb():
    return InlineKeyboardMarkup([
        ...
        [InlineKeyboardButton("📉 Разница (2 последних)",  callback_data="rep_diff")],
...
async def cmd_start(upd: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    if not row:
        ...
        if role == "admin":
            await upd.message.reply_text(
                ...
                "Вы первый пользователь и получили роль Администратора автоматически.\n"
                "Теперь вы можете добавлять товары, управлять доступом и проводить инвентаризацию.",
        ...
        for admin in db_admins():
            try:
                ...
                logger.warning(f"Не удалось уведомить {admin['tg_id']}: {e}")

        await upd.message.reply_text(
            "👋 Добро пожаловать в бот инвентаризации *КосыТату*!\n\n"
    ...
    role = row["role"]
    if role == "pending":
        await upd.message.reply_text("⏳ Ваш запрос ещё не одобрён. Пожалуйста, подождите.")
        return ConversationHandler.END
    if role == "rejected":
        await upd.message.reply_text("❌ В доступе отказано. Обратитесь к администратору.")
        return ConversationHandler.END
...
# ── Одобрение пользователей (работает вне диалога тоже) ───────────────────────
async def cb_approve(upd: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    await q.edit_message_text(f"Готово! Роль назначена: {labels.get(new_role, new_role)}")

    msg = ("❌ В доступе отказано. Обратитесь к администратору." if new_role == "rejected"
           else f"✅ Ваш доступ одобрен!\nРоль: {labels[new_role]}\n\nНапишите /start для входа.")
    try:
...
async def cb_main(upd: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    if q.data == "inv_start":
        ...
        ctx.user_data.update({"sid": sid, "inv": {}, "cat": None})
...
async def cb_inv_cat(upd: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    if not items:
        await q.edit_message_text(
            ...
            f"Администратор может добавить их через «➕ Добавить товар».",
            parse_mode=ParseMode.MARKDOWN,
            reply_markup=InlineKeyboardMarkup([
                [InlineKeyboardButton("◀️ К категориям",          callback_data="back_cats")],
                [InlineKeyboardButton("✅ Завершить инвентаризацию", callback_data="inv_finish")],
            ])
        )
        return ST_INV_CAT
    ...
    await q.edit_message_text(
        f"📂 *{cat}*\n\nВыберите позицию:\n_✅ — уже заполнено_",
        parse_mode=ParseMode.MARKDOWN,
        reply_markup=items_kb(cat, ctx.user_data.get("inv", {}))
    )
    return ST_INV_ITEM
...
async def cb_inv_item(upd: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    if item["photo_id"]:
        ...
        sent = await ctx.bot.send_photo(
            q.from_user.id, item["photo_id"],
            caption=caption, parse_mode=ParseMode.MARKDOWN, reply_markup=back_btn
        )
...
async def cb_back_to_cat(upd: Update, ctx: ContextTypes.DEFAULT_TYPE):
    """Возврат из ввода количества к списку позиций."""
    q = upd.callback_query; await q.answer()
    cat = ctx.user_data.get("cat", "")

    # Удаляем сообщение с фото если было
    photo_mid = ctx.user_data.pop("photo_msg_id", None)
    if photo_mid:
        try:
            await ctx.bot.delete_message(q.from_user.id, photo_mid)
        except Exception:
            pass
...
async def msg_qty(upd: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        await upd.message.reply_text(
            "⚠️ Введите число.\n"
            "Примеры: *42* или *0.5* (для половины пачки)",
            parse_mode=ParseMode.MARKDOWN
        )
        return ST_INV_QTY
    ...
    # Удаляем фото-сообщение если было
    photo_mid = ctx.user_data.pop("photo_msg_id", None)
    if photo_mid:
        try:
            await ctx.bot.delete_message(upd.effective_user.id, photo_mid)
        except Exception:
            pass

    cat = ctx.user_data["cat"]
    await upd.message.reply_text(f"✅ Записано: *{item['name']}* — {qstr(qty)} {item['unit']}",
                                  parse_mode=ParseMode.MARKDOWN)
    await upd.message.reply_text(
        f"📂 *{cat}*\n\nПродолжайте или завершайте:",
        parse_mode=ParseMode.MARKDOWN,
        reply_markup=items_kb(cat, ctx.user_data["inv"])
    )
    return ST_INV_ITEM


async def do_finish(q, ctx: ContextTypes.DEFAULT_TYPE, stype="inventory"):
    ...
    label = "Инвентаризация завершена" if stype == "inventory" else "Списание завершено"
    ...
    lines = [f"{emoji} *{label}*", f"👤 {name}", f"🕐 {today()}\n"]
    if cat_data:
        ...
        lines.append("_(позиции не были заполнены)_")

    summary = "\n".join(lines)
    await q.message.reply_text(summary, parse_mode=ParseMode.MARKDOWN)

    notif_label = "Новая инвентаризация" if stype == "inventory" else "Новое списание"
    for admin in db_admins():
        if admin["tg_id"] != q.from_user.id:
            try:
                await ctx.bot.send_message(
                    admin["tg_id"], f"🔔 *{notif_label}*\n\n{summary}", parse_mode=ParseMode.MARKDOWN
                )
...
async def cb_wo_cat(upd: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    await q.edit_message_text(f"✂️ *{cat}*\nСколько израсходовано?", parse_mode=ParseMode.MARKDOWN,
        reply_markup=items_kb(cat, ctx.user_data.get("inv", {}), "wo_finish", "wo_back_cats", "wo_item"))
    return ST_WO_ITEM
...
async def cb_wo_item(upd: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    caption = (f"✂️ *{item['name']}*\n📂 {item['category']}\n📏 {item['unit']}\n\n"
               f"Введите количество израсходованного:")
    back_btn = InlineKeyboardMarkup([[InlineKeyboardButton("◀️ Назад", callback_data="wo_back_to_cat")]])
    if item["photo_id"]:
        try: await q.message.delete()
        except Exception: pass
        sent = await ctx.bot.send_photo(q.from_user.id, item["photo_id"],
                caption=caption, parse_mode=ParseMode.MARKDOWN, reply_markup=back_btn)
        ctx.user_data["photo_msg_id"] = sent.message_id
    else:
        await q.edit_message_text(caption, parse_mode=ParseMode.MARKDOWN, reply_markup=back_btn)
...
async def cb_wo_back_to_cat(upd: Update, ctx: ContextTypes.DEFAULT_TYPE):
    q = upd.callback_query; await q.answer()
    cat = ctx.user_data.get("cat", "")
    photo_mid = ctx.user_data.pop("photo_msg_id", None)
    if photo_mid:
...
async def msg_wo_qty(upd: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    await upd.message.reply_text(f"✂️ *{cat}*:", parse_mode=ParseMode.MARKDOWN,
        reply_markup=items_kb(cat, ctx.user_data["inv"], "wo_finish", "wo_back_cats", "wo_item"))
    return ST_WO_ITEM
...
async def cb_add_cat(upd: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    await q.edit_message_text(
        f"📂 *{cat}*\n\n"
        f"Введите название (цвет) нового товара.\n\n"
        f"_Примеры:_\n"
        f"• Однотонный розовый\n"
        f"• Двухцветный чёрный/розовый\n"
        f"• Трёхцветный синий/белый/фиолетовый",
        parse_mode=ParseMode.MARKDOWN
    )
    return ST_ADD_NAME
...
async def msg_add_name(upd: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    await upd.message.reply_text(
        f"📸 Отправьте фото для *{ctx.user_data['new_name']}*\n\n"
...
async def _save_item(upd: Update, ctx: ContextTypes.DEFAULT_TYPE):
    cat   = ctx.user_data["new_cat"]
    name  = ctx.user_data["new_name"]
    photo = ctx.user_data.get("new_photo")
    unit  = ctx.user_data["new_unit"]
    ...
    await msg.reply_text(
        ...
        f"🏷 {name}\n"
        f"📏 Единица: {unit}\n"
        f"{'📸 С фото' if photo else '📷 Без фото'}",
        parse_mode=ParseMode.MARKDOWN,
        ...
        for p in pending:
            ...
            btns += [
                [InlineKeyboardButton(
                    f"✅ Сотрудник — {p['name'][:18]}",
                    callback_data=f"apr:employee:{p['tg_id']}"
                )],
                [
                    InlineKeyboardButton(
                        f"🏢 Директор",
                        callback_data=f"apr:director:{p['tg_id']}"
                    ),
                    InlineKeyboardButton(
                        f"❌ Отклонить",
                        callback_data=f"apr:rejected:{p['tg_id']}"
                    ),
                ],
            ]
    else:
...
async def cb_manage(upd: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    if q.data.startswith("apr:"):
        ...
        u = db_user(q.from_user.id)
        await ctx.bot.send_message(
            q.from_user.id, "👥 Больше нет заявок.", reply_markup=main_kb(u["role"])
        )
        return ST_MAIN
    return ST_MANAGE
...
async def cb_report(upd: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    # ── Текущие остатки ────────────────────────────────────────────────────────
    if data == "rep_current":
        rows = db_latest_per_item()
        if not rows:
            await q.edit_message_text(
                "📋 Данных пока нет.\nПроведите первую инвентаризацию!",
                reply_markup=back_report_kb()
            )
            return ST_REPORT
        ...
        for r in rows:
            if r["category"] != cur_cat:
                cur_cat = r["category"]
                lines.append(f"\n*{cur_cat}:*")
            lines.append(f"  • {r['name']}: {qstr(r['quantity'])} {r['unit']}")
    ...
    # ── Списания за сегодня ───────────────────────────────────────────────────
    if data == "rep_writeoffs":
        rows = db_writeoffs_today()
        if not rows:
            await q.edit_message_text(
                "✂️ Сегодня списаний не было.",
                reply_markup=back_report_kb()
            )
            return ST_REPORT
    ...
    # ── Разница ────────────────────────────────────────────────────────────────
    if data == "rep_diff":
        sessions = db_last_two_sessions()
        if len(sessions) < 2:
            await q.edit_message_text(
                "⚠️ Для сравнения нужно минимум *2* завершённые инвентаризации.",
                parse_mode=ParseMode.MARKDOWN, reply_markup=back_report_kb()
            )
            return ST_REPORT
        ...
        if not changed:
            lines.append("✅ Изменений нет — остатки совпадают")
    ...
    if data == "rep_excel":
        ...
        headers = ["Дата", "Сотрудник", "Категория", "Наименование", "Количество", "Единица"]
        ...
        for s in sessions:
            for e in db_session_entries(s["id"]):
                ...
                ws.cell(row=row_n, column=2, value=s["user_name"])
                ws.cell(row=row_n, column=3, value=e["category"])
                ws.cell(row=row_n, column=4, value=e["name"])
                ws.cell(row=row_n, column=5, value=e["quantity"])
                ws.cell(row=row_n, column=6, value=e["unit"])
                row_n += 1
        ...
        await ctx.bot.send_document(
            q.from_user.id, document=buf, filename=fname,
            caption=f"📊 Отчёт по инвентаризации\n{today()}"
        )
        await q.edit_message_text("✅ Excel-файл отправлен выше ⬆️", reply_markup=back_report_kb())
        return ST_REPORT
...
def main():
    ...
    # Глобальный обработчик одобрений (работает в любом состоянии диалога)
        ...
        states={
            ...
            ST_INV_CAT: [
                CallbackQueryHandler(
                    cb_inv_cat,
                    pattern=r"^(inv_cat:|back_cats$|back_main$|inv_finish$)"
                ),
            ],
            ST_INV_ITEM: [
                CallbackQueryHandler(
                    cb_inv_item,
                    pattern=r"^(inv_item:|back_cats$|inv_finish$)"
                ),
            ],
            ST_INV_QTY: [
            ...
            ST_WO_CAT: [
                CallbackQueryHandler(
                    cb_wo_cat,
                    pattern=r"^(wo_cat:|wo_back_cats$|back_main$|wo_finish$)"
                ),
            ],
            ST_WO_ITEM: [
                CallbackQueryHandler(
                    cb_wo_item,
                    pattern=r"^(wo_item:|wo_back_cats$|wo_finish$)"
                ),
            ],
            ST_WO_QTY: [
                MessageHandler(filters.TEXT & ~filters.COMMAND, msg_wo_qty),
                CallbackQueryHandler(cb_wo_back_to_cat, pattern=r"^wo_back_to_cat$"),
            ],
            ST_ADD_CAT: [
                CallbackQueryHandler(cb_add_cat, pattern=r"^(add_cat:|back_main$)"),
            ],
            ST_ADD_NAME: [
                MessageHandler(filters.TEXT & ~filters.COMMAND, msg_add_name),
            ],
            ST_ADD_PHOTO: [
            ...
            ST_MANAGE: [
                CallbackQueryHandler(cb_manage),
            ],
            ST_REPORT: [
                CallbackQueryHandler(cb_report),
            ],
        },
