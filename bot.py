...
def main_kb(role):
btns = [[InlineKeyboardButton(“📦 Начать инвентаризацию”, callback_data=“inv_start”)]]
...
def items_kb(category, entries: dict):
...
btns.append([InlineKeyboardButton(label, callback_data=f”inv_item:{it[‘id’]}”)])
btns.append([InlineKeyboardButton(“◀️ К категориям”,          callback_data=“back_cats”)])
btns.append([InlineKeyboardButton(“✅ Завершить инвентаризацию”, callback_data=“inv_finish”)])
return InlineKeyboardMarkup(btns)
...
if q.data == "inv_start":
    if not is_staff(u):
        return ST_MAIN
    sid = db_new_session(q.from_user.id)
...
async def do_finish(q, ctx: ContextTypes.DEFAULT_TYPE):
...
# Уведомляем других администраторов и директоров
for admin in db_admins():
    if admin["tg_id"] != q.from_user.id:
        try:
            await ctx.bot.send_message(
                admin["tg_id"],
                f"🔔 *Новая инвентаризация*\n\n{summary}",
                parse_mode=ParseMode.MARKDOWN
            )
        except Exception:
            pass
